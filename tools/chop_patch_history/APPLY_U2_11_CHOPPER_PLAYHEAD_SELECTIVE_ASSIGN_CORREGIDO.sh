...
s = p.read_text()
if 'endFrame_=-1' not in s and 'endFrame_= -1' not in s:
    s = replace_once(s, 'startFrame_=0 ;', 'startFrame_=0 ;\n\tendFrame_=-1 ;',
                     'AudioFileStreamer.cpp constructor startFrame')
if 'AudioFileStreamer::StartRangeAt' not in s:
    pattern = r'bool AudioFileStreamer::StartAt\(const Path &path,int startFrame\) \{.*?\n\} ;'
