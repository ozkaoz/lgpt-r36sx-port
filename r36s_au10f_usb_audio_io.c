...
static void poll_capture_command(const char *pcmc) {
    ...
    if (strstr(cmd, "MONITOR")) {
        ...
        if (strstr(cmd, "MONITOR=0") || strstr(cmd, "RECOVER_OUT")) {
            /* AU11U_NO_REOPEN_ON_RECORD_EXIT
            ...
            monitor_fifo_reset();
            fprintf(stderr, "AU11U_MONITOR_OFF_KEEP_ENDPOINTS_OPEN cmd=%s\n", cmd);
        }
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "AU11U_USB_AUDIO_IO_START AU11U_CAPTURE_WRITES_STAGING_AND_COUNTDOWN fifo=%s pcmp=%s pcmc=%s period_frames=%d periods=%d lowlat=%d runtime=%s\n", fifo, pcmp, pcmc, period_frames, periods, lowlat, RUNTIME_DIR);
    ...
    for (;;) {
        ...
        if (force_playback_reopen) {
            force_playback_reopen = 0;
            if (pcm >= 0) { close(pcm); pcm = -1; fprintf(stderr, "AU11U_PCM_PLAY_FORCE_REOPEN_AFTER_USB_REC_EXIT\n"); }
            ring_reset();
            good_write_streak = 0;
            mark_inactive();
            sleep_ms(120);
