...
normalize_mode() {
    case "$1" in
        ...
        WINDOWS|WINDOWS_OTG|USB_DUPLEX|USB_IN_OUT|USB_INPUT_OUTPUT|FULL_DUPLEX|USB_OUT_AUTO_MUTE|USB_IN_CAPTURE|EXTERNAL_RECORD) echo WINDOWS ;;
        *) echo LOCAL_CONSOLE ;;
    esac
}
...
case "$NORM" in
    ANDROID) POLICY=ANDROID_OTG ;;
    WINDOWS) POLICY=WINDOWS_OTG ;;
