...
void AudioMixer::SetFileRenderer(const char *path) {
    renderPath_ = path ? path : "" ;
} ;

bool AudioMixer::IsRendering() const {
    return enableRendering_ ;
} ;
...
void AudioMixer::EnableRendering(bool enable) {
    ...
    if (enable) {
        if (renderPath_.empty()) {
            ...
            Trace::Log("AudioMixer", "render enable requested with empty path");
            return ;
        }
        Trace::Log("AudioMixer", "render enable path=%s", renderPath_.c_str());
        SAFE_DELETE(writer_) ;
        writer_=new WavFileWriter(renderPath_.c_str()) ;
        enableRendering_=(writer_ && writer_->IsOpen()) ;
        if (!enableRendering_) {
            Trace::Log("AudioMixer", "render open failed path=%s", renderPath_.c_str());
            SAFE_DELETE(writer_) ;
        } else {
            Trace::Log("AudioMixer", "render open ok path=%s", renderPath_.c_str());
        }
        return ;
    }
    ...
    enableRendering_=false ;
    if (writer_) {
        Trace::Log("AudioMixer", "render closing path=%s", renderPath_.c_str());
        writer_->Close() ;
        SAFE_DELETE(writer_) ;
    }
} ;
...
bool AudioMixer::Render(fixed *buffer,int samplecount) {
    ...
    for (it->Begin(); !it->IsDone(); it->Next()) {
     ...
     if (gotData) {
         ...
         // Apply render headroom before soft/hard clipping. This avoids harsh WAV exports
         // when the live mix is close to 0 dBFS, without changing normal playback.
         float renderDamp = enableRendering_ ? 0.70794578f : 1.0f; /* -3 dB */
         c = buffer;
         for (int i = 0; i < samplecount * 2; i++) {
             fixed sample = *c;
