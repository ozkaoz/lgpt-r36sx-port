...
class PhraseView : public View {
  ...
  private:
    ...
    bool previewCurrentPhraseRow();
    bool openAssignedChopPitchEnvelope();
    bool navigatePhraseList(int delta);
    bool navigateSongAssignment(int delta);
    bool linkCreatedPhraseToNextChainSlot(int phraseIndex);
    bool handlePlainADoubleTap();
    void phraseUndoRedo();
    unsigned long lastPlainATime_;
    int lastPlainARow_;
    int lastPlainAPhrase_;
    struct clipboard {
        bool active_;
        int col_;
        int row_;
