...
bool LGPTChopperGlobalUndoRedo(ViewData *viewData, char *status, int statusLen) {
    lgptSetAssignStatus(status, statusLen, "");
    if (!viewData || !viewData->project_) {
        lgptSetAssignStatus(status, statusLen, "Undo: no project");
        return false;
    }
    if (!g_lgptPhysicalUndoSamples || !g_lgptPhysicalRedoSamples) {
        lgptSetAssignStatus(status, statusLen, "No edit undo");
        return false;
    }
    int sampleIndex = g_lgptLastDestructiveEditSampleIndex;
    if (sampleIndex < 0) {
        lgptSetAssignStatus(status, statusLen, "Undo: no sample");
        return false;
    }
    ...
    if (!source || !wav) {
        lgptSetAssignStatus(status, statusLen, "Undo source fail");
        return false;
    }
    ...
    if (!lgptRestorePhysicalSnapshotToWav(wav, g_lgptLastDestructiveEditSamplePath.c_str(),
                                          restoreSamples, restoreFrames, restoreChannels, restoreRate)) {
        lgptSetAssignStatus(status, statusLen, "Undo restore fail");
        return false;
    }
    ...
    if (slot >= 0) {
        LGPTChopperSavedState &saved = g_lgptChopperSavedStates[slot];
        saved.sourceSize = restoreFrames;
        saved.boundaryCount = restoreCount;
        if (saved.boundaryCount < 2) saved.boundaryCount = 2;
        if (saved.boundaryCount > LGPT_CHOPPER_SAVED_BOUNDARIES) saved.boundaryCount = LGPT_CHOPPER_SAVED_BOUNDARIES;
        for (int i = 0; i < LGPT_CHOPPER_SAVED_BOUNDARIES; i++) saved.boundaries[i] = 0;
        for (int i = 0; i < saved.boundaryCount; i++) saved.boundaries[i] = restoreBoundaries[i];
        saved.boundaries[0] = 0;
        saved.boundaries[saved.boundaryCount - 1] = restoreFrames > 0 ? restoreFrames - 1 : 0;
        saved.selectedChop = restoreSelected;
        if (saved.selectedChop < 0) saved.selectedChop = 0;
        if (saved.selectedChop > saved.boundaryCount - 2) saved.selectedChop = saved.boundaryCount - 2;
    ...
    lgptSetAssignStatus(status, statusLen, redo ? "Redo edit complete" : "Undo edit complete");
    return true;
}
...
SampleChopperModal::SampleChopperModal(View &view,
    ...
    : ModalView(view),
      ...
      usbCaptureLastLevel_(-1),
      usbCaptureLastLevelL_(-1),
      usbCaptureLastLevelR_(-1),
      usbCaptureLastElapsed_(-1),
      usbCaptureRequireRelease_(false),
      usbCaptureLastMask_(0),
      usbCaptureOpenGuard_(0),
      usbCaptureFilePrefixIndex_(0),
      usbCaptureFileTake_(1),
      operationPercent_(0),
      ...
      pitchScope_(0),
      normalizeTarget_(0),
      pitchLengthPercent_(100),
      initialChopRequest_(initialChop),
      openPitchEnvelopeRequest_(openPitchEnvelope),
      openUsbCaptureRequest_(openUsbCapture),
      chopReleaseMs_(0),
      selectedChop_(0),
      boundaryCount_(0),
    ...
    usbCaptureLastName_[0] = 0;
    usbCaptureLastPath_[0] = 0;
    snprintf(usbCapturePendingName_, sizeof(usbCapturePendingName_), "Sample_001.wav");
    usbCaptureFilenameCursor_ = 0;
    rebuildUsbCapturePendingName();
    ...
    if (!openUsbCaptureRequest_) {
        prepareWaveformPreview();
    } else {
        /* AU11U_USB_REC_NO_WAVEFORM_COLD_INIT
           USB-C RECORD is an input sampler menu and does not need to parse or
           allocate waveform preview data on entry. Skipping waveform init removes
           one first-use crash vector when opening RECORD from Instrument while the
           project has just been stopped. */
        hasWaveform_ = false;
        boundaryCount_ = 0;
        setStatus("USB-C RECORD");
    }
    if (initialChopRequest_ >= 0 && boundaryCount_ > 1) {
        selectedChop_ = clampInt(initialChopRequest_, 0, boundaryCount_ - 2);
        cursorFrame_ = boundaries_[selectedChop_];
        centerViewOnCursor();
    }
    if (openPitchEnvelopeRequest_) {
        pitchMode_ = true;
        trimMode_ = false;
        pitchScope_ = 1;
        normalizeTarget_ = 1;
        pitchEditParam_ = 0;
        setStatus("Pitch/Env chop");
    }
    // AU10Z_OPEN_USB_CAPTURE_FROM_INSTRUMENT
    if (openUsbCaptureRequest_) {
        openUsbCaptureMenu();
    }
    publishOverlayState();
...
// AU10Y compile fix: destructor calls this helper before its definition below.
static void au10yRecordModalSentinel(int active);
...
void SampleChopperModal::OnFocus() {
#if defined(PLATFORM_TREEFROG)
    if (usbCaptureMenuActive_) { g_chopperOverlayActive = 0; tf_rect(0, 0, TF_W, TF_H, tf_rgb565(4, 3, 8)); }
#endif
    publishOverlayState(); isDirty_ = true;
}
void SampleChopperModal::OnPlayerUpdate(PlayerEventType, unsigned int currentTick) {
    if (!usbCaptureMenuActive_) return;
    int level = TreeFrogUac2Bridge_GetUsbCaptureLevelPercent();
    int levelL = TreeFrogUac2Bridge_GetUsbCaptureLevelLeftPercent();
    int levelR = TreeFrogUac2Bridge_GetUsbCaptureLevelRightPercent();
    int elapsed = TreeFrogUac2Bridge_GetUsbCaptureElapsedSeconds();
    if (level != usbCaptureLastLevel_ || levelL != usbCaptureLastLevelL_ || levelR != usbCaptureLastLevelR_ || elapsed != usbCaptureLastElapsed_ || (currentTick % 2) == 0) {
        usbCaptureLastLevel_ = level; usbCaptureLastLevelL_ = levelL; usbCaptureLastLevelR_ = levelR; usbCaptureLastElapsed_ = elapsed;
        isDirty_ = true;
        if (g_au10y_usb_record_draw_guard) return;
        g_au10y_usb_record_draw_guard = 1;
        DrawView();
        w_.Flush();
#if defined(PLATFORM_TREEFROG)
        TreeFrogForceVideoRefresh();
#endif
        g_au10y_usb_record_draw_guard = 0;
    }
}
...
// U2.25: fixes live progress visibility, Pitch/Envelope preview, L1+X undo/redo, and Sample/Chop scope.
...
void SampleChopperModal::drawPitchScreen(GUITextProperties &props) {
    ...
    drawStringAbs(1, 26, openPitchEnvelopeRequest_ ? "R1+B back Phrase    R2+LR chop" : "L1+R1 exit          R2+LR chop", props);
}
...
static void au10yRecordModalSentinel(int active) {
#if defined(PLATFORM_TREEFROG)
    const char *p = "/tmp/r36sx_au10y_usb_rec_modal";
    if (active) {
...
static int au10yAllowedFileChar(char c) {
    if (c >= 'A' && c <= 'Z') return 1;
    if (c >= 'a' && c <= 'z') return 1;
    if (c >= '0' && c <= '9') return 1;
    return c == '_' || c == '-' || c == '.';
...
/* AU10Y_SAFE_OPEN_USB_RECORD: USB-C RECORD stops active Chopper preview before opening. */
void SampleChopperModal::toggleUsbCapture() { openUsbCaptureMenu(); }

void SampleChopperModal::openUsbCaptureMenu() {
    /* AU10Y: ProcessButtonMask now blocks menu opening during active preview.
       Do not call Player::StopStreaming() here; clear only visual preview state
       to avoid reentrant audio teardown while entering the record screen. */
...
#if defined(PLATFORM_TREEFROG)
    g_chopperOverlayActive = 0;
    tf_rect(0, 0, TF_W, TF_H, tf_rgb565(4, 3, 8));
#endif
    isDirty_ = true;
}
...
void SampleChopperModal::closeUsbCaptureMenu() {
    ...
    TreeFrogUac2Bridge_SetUsbMonitor(0);
    au10yRecordModalSentinel(0);
    setStatus("USB REC closed");
    isDirty_ = true;
}

void SampleChopperModal::exitUsbCaptureToInstrument() {
    // AU11U_USB_RECORD_R1_LEFT_RETURN_INSTRUMENT
    if (usbCaptureRecording_) {
        TreeFrogUac2Bridge_StopUsbCapture();
        usbCaptureRecording_ = false;
    }
...
void SampleChopperModal::activateUsbCaptureMenuItem() {
    switch (usbCaptureMenuSelection_) {
        case 0: enterUsbCaptureFilenameEdit(); break;
        case 1: startUsbCaptureFromMenu(); break;
        case 2: stopUsbCaptureFromMenu(); break;
        case 3: saveUsbCaptureFromMenu(); break;
        case 4: previewUsbCaptureFromMenu(); break;
        case 5: discardUsbCaptureFromMenu(); break;
        case 6: toggleUsbCaptureMonitor(); break;
        default: break;
    }
}
...
void SampleChopperModal::sanitizeUsbCapturePendingName() {
    if (!usbCapturePendingName_[0]) snprintf(usbCapturePendingName_, sizeof(usbCapturePendingName_), "Sample_001.wav");
    char clean[96]; int j = 0;
    for (int i = 0; usbCapturePendingName_[i] && j < 90; ++i) {
        char c = usbCapturePendingName_[i];
        if (c == ' ') c = '_';
        if (au10yAllowedFileChar(c)) clean[j++] = c;
    }
    clean[j] = 0;
    if (!clean[0]) snprintf(clean, sizeof(clean), "Sample_001.wav");
    /* AU10Y: normalize extension. Keep one final .wav only. If user creates
       Sample.wav.wav or Sample.WAV.wav, collapse to Sample.wav. */
    char base[96]; snprintf(base, sizeof(base), "%s", clean);
    for (;;) {
        int blen = (int)strlen(base);
        if (blen > 8 && strcasecmp(base + blen - 8, ".wav.wav") == 0) { base[blen - 4] = 0; continue; }
        break;
    }
    int blen = (int)strlen(base);
    if (!(blen >= 4 && strcasecmp(base + blen - 4, ".wav") == 0)) strncat(base, ".wav", sizeof(base) - strlen(base) - 1);
    snprintf(usbCapturePendingName_, sizeof(usbCapturePendingName_), "%s", base);
    int len = (int)strlen(usbCapturePendingName_);
    if (usbCaptureFilenameCursor_ < 0) usbCaptureFilenameCursor_ = 0;
    if (usbCaptureFilenameCursor_ >= len) usbCaptureFilenameCursor_ = len > 0 ? len - 1 : 0;
}

void SampleChopperModal::resetUsbCaptureFilenameDefault() {
    snprintf(usbCapturePendingName_, sizeof(usbCapturePendingName_), "Sample_001.wav");
    usbCaptureFilenameCursor_ = 0;
    setStatus("USB REC filename default"); isDirty_ = true;
}

void SampleChopperModal::enterUsbCaptureFilenameEdit() {
    sanitizeUsbCapturePendingName(); usbCaptureFilenameEdit_ = true; usbCaptureFilenameCursor_ = 0;
    setStatus("Edit filename: START OK, A+arrows fast"); isDirty_ = true;
}

void SampleChopperModal::exitUsbCaptureFilenameEdit(bool accept) {
    if (accept) { sanitizeUsbCapturePendingName(); setStatus("Filename accepted"); }
    else setStatus("Filename edit closed");
    usbCaptureFilenameEdit_ = false; isDirty_ = true;
}

void SampleChopperModal::moveUsbCaptureFilenameCursor(int delta) {
    sanitizeUsbCapturePendingName(); int len = (int)strlen(usbCapturePendingName_);
    if (len <= 0) return; usbCaptureFilenameCursor_ += delta;
    if (usbCaptureFilenameCursor_ < 0) usbCaptureFilenameCursor_ = len - 1;
    if (usbCaptureFilenameCursor_ >= len) usbCaptureFilenameCursor_ = 0;
    isDirty_ = true;
}

void SampleChopperModal::editUsbCaptureFilenameChar(int delta) {
    sanitizeUsbCapturePendingName();
    static const char table[] = " ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-.";
    int len = (int)strlen(usbCapturePendingName_);
    if (len <= 0) { resetUsbCaptureFilenameDefault(); return; }
    if (usbCaptureFilenameCursor_ < 0) usbCaptureFilenameCursor_ = 0;
    if (usbCaptureFilenameCursor_ >= len) usbCaptureFilenameCursor_ = len - 1;
    char c = usbCapturePendingName_[usbCaptureFilenameCursor_];
    int n = (int)strlen(table), idx = 1;
    for (int i = 0; i < n; ++i) if (table[i] == c) { idx = i; break; }
    idx += delta; while (idx < 0) idx += n; idx %= n;
    c = table[idx]; if (c == ' ') c = '_';
    usbCapturePendingName_[usbCaptureFilenameCursor_] = c;
    sanitizeUsbCapturePendingName(); isDirty_ = true;
}

void SampleChopperModal::deleteUsbCaptureFilenameChar() {
    sanitizeUsbCapturePendingName();
    int len = (int)strlen(usbCapturePendingName_);
    if (len <= 1) { resetUsbCaptureFilenameDefault(); return; }
    if (usbCaptureFilenameCursor_ < 0) usbCaptureFilenameCursor_ = 0;
    if (usbCaptureFilenameCursor_ >= len) usbCaptureFilenameCursor_ = len - 1;
    memmove(&usbCapturePendingName_[usbCaptureFilenameCursor_], &usbCapturePendingName_[usbCaptureFilenameCursor_ + 1], (size_t)(len - usbCaptureFilenameCursor_));
    if (usbCaptureFilenameCursor_ >= (int)strlen(usbCapturePendingName_) && usbCaptureFilenameCursor_ > 0) usbCaptureFilenameCursor_--;
    sanitizeUsbCapturePendingName(); isDirty_ = true;
}

void SampleChopperModal::toggleUsbCaptureFilenameCase() {
    sanitizeUsbCapturePendingName();
    int len = (int)strlen(usbCapturePendingName_);
    if (len <= 0) return;
    if (usbCaptureFilenameCursor_ < 0) usbCaptureFilenameCursor_ = 0;
...
void SampleChopperModal::rebuildUsbCapturePendingName() { sanitizeUsbCapturePendingName(); }
void SampleChopperModal::nudgeUsbCaptureTake(int delta) { (void)delta; enterUsbCaptureFilenameEdit(); }
void SampleChopperModal::cycleUsbCapturePrefix() { enterUsbCaptureFilenameEdit(); }

void SampleChopperModal::startUsbCapture() { startUsbCaptureFromMenu(); }
void SampleChopperModal::stopUsbCapture() { stopUsbCaptureFromMenu(); }

void SampleChopperModal::startUsbCaptureFromMenu() {
    stopSamplePreview();
    sanitizeUsbCapturePendingName();
    usbCaptureMonitor_ = true;
    TreeFrogUac2Bridge_SetUsbMonitor(1); /* AU11 monitor-on; UI no longer opens FIFO synchronously. */
    ...
    std::string absPath = "/mnt/sdcard/lgpt/usbrecs/"; absPath += name;
    snprintf(usbCaptureLastPath_, sizeof(usbCaptureLastPath_), "%s", absPath.c_str());
    if (TreeFrogUac2Bridge_StartUsbCapture(absPath.c_str(), 120)) {
        usbCaptureRecording_ = true;
        snprintf(usbCaptureLastName_, sizeof(usbCaptureLastName_), "%s", name);
        setStatus("REC ACTIVE: 120s countdown");
    } else setStatus("USB REC start failed");
    isDirty_ = true;
}

void SampleChopperModal::stopUsbCaptureFromMenu() {
    if (TreeFrogUac2Bridge_StopUsbCapture()) { usbCaptureRecording_ = false; setStatus("USB REC stopped: Save or Discard"); }
    else setStatus("USB REC stop failed");
    isDirty_ = true;
}

void SampleChopperModal::saveUsbCaptureFromMenu() { importLastUsbCapture(); }

void SampleChopperModal::discardUsbCaptureFromMenu() {
    TreeFrogUac2Bridge_DiscardUsbCapture();
    usbCaptureRecording_ = false; usbCaptureLastName_[0] = 0; usbCaptureLastPath_[0] = 0;
    setStatus("USB REC discarded"); isDirty_ = true;
}

void SampleChopperModal::toggleUsbCaptureMonitor() {
    usbCaptureMonitor_ = !usbCaptureMonitor_;
    TreeFrogUac2Bridge_SetUsbMonitor(usbCaptureMonitor_ ? 1 : 0);
    setStatus(usbCaptureMonitor_ ? "USB monitor requested" : "USB monitor off");
    isDirty_ = true;
}

void SampleChopperModal::importLastUsbCapture() {
    char name[96]; name[0] = 0;
    char capturedPath[256]; capturedPath[0] = 0;
    if (!TreeFrogUac2Bridge_GetLastCaptureName(name, sizeof(name)) && usbCaptureLastName_[0]) snprintf(name, sizeof(name), "%s", usbCaptureLastName_);
    if (!TreeFrogUac2Bridge_GetLastCapturePath(capturedPath, sizeof(capturedPath)) && usbCaptureLastPath_[0]) snprintf(capturedPath, sizeof(capturedPath), "%s", usbCaptureLastPath_);
    if (!name[0]) { setStatus("No USB recording yet"); return; }
    if (capturedPath[0]) {
        std::string dstLogical = "samples:"; dstLogical += name;
        Path srcPath(capturedPath);
        Path dstPath(dstLogical.c_str());
        FileSystemService fss;
        if (fss.Copy(srcPath, dstPath) < 0) { setStatus("USB rec copy to samples failed"); return; }
    }
    int idx = SamplePool::GetInstance()->Reassign(std::string(name), false);
    if (idx < 0) { setStatus("USB rec not ready/import failed"); return; }
    if (viewData_ && viewData_->project_) {
        InstrumentBank *bank = viewData_->project_->GetInstrumentBank();
        if (bank) { I_Instrument *ii = bank->GetInstrument(instrumentIndex_); if (ii && ii->GetType() == IT_SAMPLE) ((SampleInstrument *)ii)->AssignSample(idx); }
    }
    sampleIndex_ = idx; sampleName_ = name; samplePath_ = "samples:"; samplePath_ += name;
    loadSampleByIndex(idx, "USB rec import");
    initializeChopsIfNeeded(); selectedChop_ = 0; cursorFrame_ = 0;
    ...
    // AU11M: leaving USB-C RECORD after save/load must restore LGPT->USB path.
    usbCaptureMonitor_ = false;
    TreeFrogUac2Bridge_SetUsbMonitor(0);
...
void SampleChopperModal::previewUsbCaptureFromMenu() {
    char name[96]; name[0] = 0;
    if (!TreeFrogUac2Bridge_GetLastCaptureName(name, sizeof(name)) && usbCaptureLastName_[0]) snprintf(name, sizeof(name), "%s", usbCaptureLastName_);
    if (!name[0]) { setStatus("No USB recording to preview"); isDirty_ = true; return; }
    char capturedPath[256]; capturedPath[0] = 0;
    if (TreeFrogUac2Bridge_GetLastCapturePath(capturedPath, sizeof(capturedPath))) {
        std::string dstLogical = "samples:"; dstLogical += name;
        Path srcPath(capturedPath);
        Path dstPath(dstLogical.c_str());
        FileSystemService fss;
        fss.Copy(srcPath, dstPath);
    }
    int idx = SamplePool::GetInstance()->Reassign(std::string(name), false);
    if (idx < 0) { setStatus("USB preview import failed"); isDirty_ = true; return; }
    sampleIndex_ = idx; sampleName_ = name; samplePath_ = "samples:"; samplePath_ += name;
...
void SampleChopperModal::drawUsbCaptureMenu(GUITextProperties &props) {
    /* AU11U_USB_RECORD_VISUAL_REC_INDICATOR AU11U_USB_RECORD_COUNTDOWN_120 */
    if (!usbCaptureMenuActive_) return;
#if defined(PLATFORM_TREEFROG)
    g_chopperOverlayActive = 0;
    g_chopperPreviewActive = 0;
    ...
    View::Clear();
    sanitizeUsbCapturePendingName();
    const char *items[7] = { "File", "Start recording", "Stop recording", "Save/load recording", "Preview recorded sample", "Discard recording", "Monitor input to console" };
    int level = TreeFrogUac2Bridge_GetUsbCaptureLevelPercent();
    int levelL = TreeFrogUac2Bridge_GetUsbCaptureLevelLeftPercent();
    int levelR = TreeFrogUac2Bridge_GetUsbCaptureLevelRightPercent();
    int elapsed = TreeFrogUac2Bridge_GetUsbCaptureElapsedSeconds();
    usbCaptureLastLevel_ = level; usbCaptureLastLevelL_ = levelL; usbCaptureLastLevelR_ = levelR; usbCaptureLastElapsed_ = elapsed;
    const char *st = TreeFrogUac2Bridge_GetUsbCaptureStatusText();
    char line[96];
    SetColor(CD_HILITE1); props.invert_ = true;
    drawStringAbs(0, 0, " P G  SCPI-R  M TT      USB-C RECORD    ", props);
    props.invert_ = false;
    SetColor(CD_NORMAL);
    drawStringAbs(0, 1,  "+--------------------------------------+", props);
    for (int y = 2; y < 28; ++y) { drawStringAbs(0, y, "|", props); drawStringAbs(39, y, "|", props); }
    drawStringAbs(0, 28, "+--------------------------------------+", props);
    snprintf(line, sizeof(line), "Mode : SCPI-R isolated USB capture"); drawStringAbs(2, 3, line, props);
    int statusRecording = (st && strstr(st, "recording"));
    if (usbCaptureRecording_ && !statusRecording && st && (strstr(st, "saved") || strstr(st, "idle") || strstr(st, "stopped") || strstr(st, "duration"))) usbCaptureRecording_ = false;
    int remaining = 120 - elapsed; if (remaining < 0) remaining = 0; if (remaining > 120) remaining = 120;
    snprintf(line, sizeof(line), "Status: %-29.29s", st ? st : "idle"); drawStringAbs(2, 5, line, props);
    if (statusRecording || usbCaptureRecording_) {
        SetColor(CD_HILITE2); props.invert_ = true;
        snprintf(line, sizeof(line), "  REC ACTIVE  remaining %03d s       ", remaining);
        drawStringAbs(2, 4, line, props); props.invert_ = false; SetColor(CD_NORMAL);
    } else {
        SetColor(CD_NORMAL); drawStringAbs(2, 4, "  REC standby  max take 120 s        ", props);
    }
    if (usbCaptureFilenameEdit_) {
        char edit[40]; snprintf(edit, sizeof(edit), "%-32.32s", usbCapturePendingName_);
        int c = usbCaptureFilenameCursor_; if (c < 0) c = 0; if (c > 31) c = 31;
        snprintf(line, sizeof(line), "File : %-32.32s", edit); drawStringAbs(2, 6, line, props);
        snprintf(line, sizeof(line), "       %*s^", c, ""); drawStringAbs(2, 7, line, props);
    } else { snprintf(line, sizeof(line), "File : %-30.30s", usbCapturePendingName_); drawStringAbs(2, 6, line, props); }
    int barsL = (levelL * 24 + 50) / 100; if (barsL < 0) barsL = 0; if (barsL > 24) barsL = 24;
    int barsR = (levelR * 24 + 50) / 100; if (barsR < 0) barsR = 0; if (barsR > 24) barsR = 24;
    char meterL[32], meterR[32];
    for (int i = 0; i < 24; ++i) { meterL[i] = (i < barsL) ? '|' : ' '; meterR[i] = (i < barsR) ? '|' : ' '; }
    meterL[24] = 0; meterR[24] = 0;
    snprintf(line, sizeof(line), "L [%s] %3d%%", meterL, levelL); drawStringAbs(2, 9, line, props);
    snprintf(line, sizeof(line), "R [%s] %3d%%", meterR, levelR); drawStringAbs(2, 10, line, props);
    snprintf(line, sizeof(line), "Timer: %03d s left  Monitor: console", remaining); drawStringAbs(2, 11, line, props);
    drawStringAbs(2, 12, "Preview plays saved WAV locally", props);
    if (usbCaptureFilenameEdit_) {
        SetColor(CD_HILITE2); props.invert_ = true; drawStringAbs(3, 14, " Filename editor ", props); props.invert_ = false; SetColor(CD_NORMAL);
        drawStringAbs(3, 16, "LEFT/RIGHT move cursor", props);
        drawStringAbs(3, 17, "UP/DOWN change character", props);
        drawStringAbs(3, 18, "START accept   B exit", props);
        drawStringAbs(3, 19, "Y delete  X case  A+arrows fast", props);
    } else {
        for (int i = 0; i < 7; ++i) {
            props.invert_ = (i == usbCaptureMenuSelection_);
            SetColor(i == usbCaptureMenuSelection_ ? CD_HILITE2 : CD_NORMAL);
            if (i == 0) snprintf(line, sizeof(line), "%c %-8.8s %-20.20s", i == usbCaptureMenuSelection_ ? '>' : ' ', items[i], usbCapturePendingName_);
            else if (i == 6) snprintf(line, sizeof(line), "%c %-24.24s [%s]", i == usbCaptureMenuSelection_ ? '>' : ' ', items[i], usbCaptureMonitor_ ? "ON" : "OFF");
            else snprintf(line, sizeof(line), "%c %-32.32s", i == usbCaptureMenuSelection_ ? '>' : ' ', items[i]);
            drawStringAbs(3, 13 + i, line, props); props.invert_ = false;
        }
        SetColor(CD_HILITE1);
        drawStringAbs(2, 22, "A select     B close", props);
        drawStringAbs(2, 23, "UP/DOWN item Y discard", props);
...
void SampleChopperModal::drawOperationOverlay(GUITextProperties &props) {
    ...
    if (operationPercent_ >= 100) {
        ...
        drawStringAbs(x, y + 4, "|      Press A to continue       |", props);
    } else {
...
void SampleChopperModal::ProcessButtonMask(unsigned short mask, bool pressed) {
    if (!pressed) {
        if (usbCaptureMenuActive_) { usbCaptureRequireRelease_ = false; usbCaptureLastMask_ = 0; }
        return;
    }
    if (usbCaptureMenuActive_) {
        if (usbCaptureOpenGuard_ > 0) { usbCaptureOpenGuard_--; return; }
        if (usbCaptureRequireRelease_) return;
        if (usbCaptureLastMask_ == mask) return;
        usbCaptureLastMask_ = mask;
    }
    ...
    /* AU10Y: L2+A must not open USB-C RECORD while any audio transport is active.
       AU10J fixed Chopper sample preview, but real hardware still crashed when
    ...
    if (usbCaptureMenuActive_) {
        if (usbCaptureFilenameEdit_) {
            /* AU10Y filename editor: START accepts; A+arrows is fast navigation/change. */
            if (a && left && !(right || up || down || b || x || y || l1 || r1 || l2 || r2 || select || start)) { moveUsbCaptureFilenameCursor(-5); DrawView(); return; }
            if (a && right && !(left || up || down || b || x || y || l1 || r1 || l2 || r2 || select || start)) { moveUsbCaptureFilenameCursor(5); DrawView(); return; }
            if (a && up && !(left || right || down || b || x || y || l1 || r1 || l2 || r2 || select || start)) { editUsbCaptureFilenameChar(5); DrawView(); return; }
            if (a && down && !(left || right || up || b || x || y || l1 || r1 || l2 || r2 || select || start)) { editUsbCaptureFilenameChar(-5); DrawView(); return; }
            if (left && !(right || up || down || a || b || x || y || l1 || r1 || l2 || r2 || select || start)) { moveUsbCaptureFilenameCursor(-1); DrawView(); return; }
            if (right && !(left || up || down || a || b || x || y || l1 || r1 || l2 || r2 || select || start)) { moveUsbCaptureFilenameCursor(1); DrawView(); return; }
            if (up && !(left || right || down || a || b || x || y || l1 || r1 || l2 || r2 || select || start)) { editUsbCaptureFilenameChar(1); DrawView(); return; }
            if (down && !(left || right || up || a || b || x || y || l1 || r1 || l2 || r2 || select || start)) { editUsbCaptureFilenameChar(-1); DrawView(); return; }
            if (start && !(left || right || up || down || a || b || x || y || l1 || r1 || l2 || r2 || select)) { exitUsbCaptureFilenameEdit(true); DrawView(); return; }
            if (a && !(left || right || up || down || b || x || y || l1 || r1 || l2 || r2 || select || start)) { setStatus("Use START to accept filename"); DrawView(); return; }
            if (b && !(left || right || up || down || a || x || y || l1 || r1 || l2 || r2 || select || start)) { exitUsbCaptureFilenameEdit(false); DrawView(); return; }
            if (x && !(left || right || up || down || a || b || y || l1 || r1 || l2 || r2 || select || start)) { toggleUsbCaptureFilenameCase(); DrawView(); return; }
        ...
        if (y && !(left || right || up || down || a || b || x || l1 || r1 || l2 || r2 || select)) { discardUsbCaptureFromMenu(); DrawView(); return; }
        if (b && !(left || right || up || down || a || x || y || l1 || r1 || l2 || r2 || select)) { closeUsbCaptureMenu(); DrawView(); return; }
        return;
    }
    ...
    if (l1 && x && !(left || right || up || down || a || b || y || r1 || l2 || r2 || select)) {
        undoLastDestructiveCrop(); return;
    }

    if (pitchMode_) {
        if (openPitchEnvelopeRequest_ && r1 && b && !(left || right || up || down || a || x || y || l1 || l2 || r2 || select)) {
            stopSamplePreview();
            EndModal(0);
            isDirty_ = true;
            return;
