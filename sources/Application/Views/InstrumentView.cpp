...
//	position._y+=View::fieldSpaceHeight_;
	...
	int au11uSampleSize = instrument->GetSampleSize();
	if (au11uSampleSize <= 0) au11uSampleSize = 1;

	position._y+=1 ;
	v=instrument->FindVariable(SIP_START) ;
	f1=new UIBigHexVarField(position,*v,7,"start: %7.7X",0,au11uSampleSize-1,16) ;
	T_SimpleList<UIField>::Insert(f1) ;

	position._y+=1 ;
	v=instrument->FindVariable(SIP_LOOPSTART) ;
	f1=new UIBigHexVarField(position,*v,7,"loop start: %7.7X",0,au11uSampleSize-1,16) ;
	T_SimpleList<UIField>::Insert(f1) ;

	position._y+=1 ;
	v=instrument->FindVariable(SIP_END) ;
	f1=new UIBigHexVarField(position,*v,7,"loop end: %7.7X",0,au11uSampleSize-1,16) ;
	T_SimpleList<UIField>::Insert(f1) ;
