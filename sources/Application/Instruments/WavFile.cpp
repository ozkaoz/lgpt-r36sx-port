...
//Trace::Dump("Loading sample from %s",path) ;
	...
	if (nChannels == 0 || bitPerSample == 0 || size == 0) {
		Trace::Error("Bad WAV data size/channels: size=%d channels=%d bps=%d", size, nChannels, bitPerSample);
		delete wav ;
		return 0 ;
	}
	wav->size_=size/nChannels/bitPerSample ; // Size in samples (stereo/16bits)
	if (wav->size_ <= 0) {
		Trace::Error("Bad WAV sample count: data bytes=%d", size);
		delete wav ;
		return 0 ;
	}
