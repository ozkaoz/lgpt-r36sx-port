...
extern bool LGPTChopperGetChopRangeForSampleIndex(int sampleIndex, int chopIndex, int *startFrame, int *endFrameExclusive);
#include <assert.h>
...
bool SampleInstrument::Start(int channel,unsigned char midinote,bool cleanstart)
{
  ...
  int lgptRuntimeChop = -1;
  int lgptRuntimeChopStart = 0;
  int lgptRuntimeChopEnd = 0;
  unsigned char playbackNote = midinote;
  if (midinote < 100 && LGPTChopperGetChopRangeForSampleIndex(GetSampleIndex(), (int)midinote,
...
/*	 if (loopmode==SILM_OSCFINE) {
     ...
     int isSliced = (lgptRuntimeChop < 0) && (slices_->GetInt() > 1);
     if (isSliced) {
	 ...
	 switch (loopmode) {
      ...
      break ;
        ...
        fineTune /= float(0x80);
        int offset = playbackNote - rootNote;
        if (isSliced) {
