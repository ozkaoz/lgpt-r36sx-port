...
int PlayerMixer::GetMasterPeakLeft() {
    MixerService *ms=MixerService::GetInstance() ;
    return ms->GetMasterPeakLeft() ;
}

int PlayerMixer::GetMasterPeakRight() {
    MixerService *ms=MixerService::GetInstance() ;
    return ms->GetMasterPeakRight() ;
}

int PlayerMixer::GetChannelPeak(int channel) {
    if ((channel<0)||(channel>=SONG_CHANNEL_COUNT)) return 0 ;
    return channel_[channel]->GetPeak() ;
}

bool PlayerMixer::IsChannelClipped(int channel) {
    if ((channel<0)||(channel>=SONG_CHANNEL_COUNT)) return false ;
    return channel_[channel]->GetClipped() ;
}
