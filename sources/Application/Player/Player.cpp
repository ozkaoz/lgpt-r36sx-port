...
#ifndef TREEFROG_STARTTRACE_REAL_LOGGER_V133
...
Player::Player() {
    ...
    exportSaveSongX_=0 ;
    exportSaveSongY_=0 ;
    exportSaveSongOffset_=0 ;
    exportSaveChainRow_=0 ;
    exportSavePlayMode_=PM_SONG ;
    exportSaveSequencerMode_=SM_SONG ;
...
void Player::Start(PlayMode mode,bool forceSongMode) {
#if defined(PLATFORM_TREEFROG)
    TreeFrogStartTrace("V14_Player_Start_ARM_AUDIO");
    if (!offlineRendering_) TreeFrogAudioSetPlaybackArmed(1);
#endif
  ...
  switch(viewData_->playMode_)
  {
		case PM_SONG:
    {
      /* V133_START_ONLY_PLAYABLE_CHANNELS */
          for (int i=0;i<8;i++) {
             if (isPlayable(playPos, i, viewData_->chainRow_)) {
                mixer_->StartChannel(i) ;
                updateSongPos(playPos, i, viewData_->chainRow_);
             } else {
                /* V133: no llamamos StopChannel(i) aquí; en algunas ramas no existe como método directo.
                   El bucle de limpieza anterior ya detuvo/limpió los canales. */
                viewData_->currentPlayChain_[i] = 0xFF;
                viewData_->currentPlayPhrase_[i] = 0xFF;
                viewData_->chainPlayPos_[i] = 0;
                viewData_->phrasePlayPos_[i] = 0;
                timeToStart_[i] = 0;
                timeToLive_[i] = 0;
             }
          }
  ...
  ProcessCommands() ;
	...
	TreeFrogStartTrace("V133_Player_Start_UNLOCK_BYPASS");
#if !defined(PLATFORM_TREEFROG)
    mixer_->Unlock() ;
#endif
...
void Player::Stop() {
#if defined(PLATFORM_TREEFROG)
    TreeFrogStartTrace("V14_Player_Stop_DISARM_AUDIO");
    if (!offlineRendering_) TreeFrogAudioSetPlaybackArmed(0);
#endif

    TreeFrogStartTrace("V133_Player_Stop_ENTER");
    TreeFrogStartTrace("Player::Stop ENTER");

	TreeFrogStartTrace("V133_Player_Stop_LOCK_BYPASS");
#if !defined(PLATFORM_TREEFROG)
    mixer_->Lock() ;
#endif
	...
	TreeFrogStartTrace("V133_Player_Stop_UNLOCK_BYPASS");
#if !defined(PLATFORM_TREEFROG)
    mixer_->Unlock() ;
#endif
}
...
int Player::findFirstSongRow() {
    if (!viewData_ || !viewData_->song_) return -1;

    for (int row = 0; row < 256; row++) {
        for (int channel = 0; channel < SONG_CHANNEL_COUNT; channel++) {
            unsigned char chain = viewData_->song_->data_[8 * row + channel];
            if (chain != 0xFF) {
                unsigned char firstPhrase = viewData_->song_->chain_->data_[16 * chain];
                if (firstPhrase != 0xFF) return row;
            }
        }
    }
    return -1;
...
int Player::findNextSongRow(int startRow) {
    if (!viewData_ || !viewData_->song_) return -1;
    if (startRow < 0) startRow = 0;

    for (int row = startRow; row < 256; row++) {
        for (int channel = 0; channel < SONG_CHANNEL_COUNT; channel++) {
            unsigned char chain = viewData_->song_->data_[8 * row + channel];
            if (chain != 0xFF) {
                unsigned char firstPhrase = viewData_->song_->chain_->data_[16 * chain];
...
bool Player::hasPlayingChannels() {
    for (int i = 0; i < SONG_CHANNEL_COUNT; i++) {
        if (mixer_->IsChannelPlaying(i)) return true;
    }
    return false;
}

bool Player::BeginSongExportToWav(int renderMode) {
    if (!viewData_ || !project_ || renderMode <= 0) return false;
    if (exportActive_) return false;

    int firstRow = findFirstSongRow();
    int lastRow = findLastSongRow();
    if (firstRow < 0 || lastRow < firstRow) {
        Trace::Log("WAVExport", "empty song; nothing to export");
        exportOk_ = false;
        return false;
    }
    ...
    exportSaveSongX_ = viewData_->songX_;
    exportSaveSongY_ = viewData_->songY_;
    exportSaveSongOffset_ = viewData_->songOffset_;
    exportSaveChainRow_ = viewData_->chainRow_;
    exportSavePlayMode_ = viewData_->playMode_;
    exportSaveSequencerMode_ = sequencerMode_;

    viewData_->songX_ = 0;
    viewData_->songY_ = firstRow;
    viewData_->songOffset_ = 0;
    viewData_->chainRow_ = 0;

    offlineRendering_ = true;
    offlineSongLastRow_ = lastRow;

    exportActive_ = true;
    exportOk_ = false;
    exportRenderMode_ = renderMode;
    exportFirstRow_ = firstRow;
    exportLastRow_ = lastRow;
    exportSlice_ = 0;
    exportMaxSlices_ = ((lastRow - firstRow + 2) * 16 * 96) + 2048;
    if (exportMaxSlices_ < 4096) exportMaxSlices_ = 4096;

    MixerService *ms = MixerService::GetInstance();
    ms->SetExportContext(project_);
    ms->SetRenderMode(renderMode);
    if (!ms->BeginOfflineRendering(renderMode)) {
        finishSongExport(false);
        Trace::Log("WAVExport", "failed to begin offline export mode=%d", renderMode);
        return false;
    }

    Start(PM_SONG, false);
    Trace::Log("WAVExport", "offline export begin mode=%d firstRow=%d lastRow=%d maxSlices=%d", renderMode, firstRow, lastRow, exportMaxSlices_);
    return true;
}

bool Player::ContinueSongExport(int budgetSlices) {
    if (!exportActive_) return exportOk_;
    if (budgetSlices <= 0) budgetSlices = 1;

    MixerService *ms = MixerService::GetInstance();
    int processed = 0;
    while (exportActive_ && processed < budgetSlices) {
        if (!IsRunning()) {
            finishSongExport(true);
            break;
        }

        ms->RenderOfflineSlice();
        exportSlice_++;
        processed++;

        if (!hasPlayingChannels()) {
            int nextRow = viewData_ ? findNextSongRow(viewData_->songY_ + 1) : -1;
            if (nextRow >= 0 && nextRow <= exportLastRow_) {
                viewData_->songY_ = nextRow;
                viewData_->songOffset_ = 0;
                viewData_->chainRow_ = 0;
                Start(PM_SONG, false);
                Trace::Log("WAVExport", "offline export jump to next active row=%d", nextRow);
            } else {
                finishSongExport(true);
                break;
            }
        ...
        if (exportSlice_ >= exportMaxSlices_) {
            finishSongExport(false);
            Trace::Log("WAVExport", "offline export hit safety limit mode=%d slices=%d", exportRenderMode_, exportSlice_);
            break;
        }
    }
    return exportOk_;
...
void Player::finishSongExport(bool ok) {
    if (!exportActive_) return;

    if (IsRunning()) Stop();
    MixerService::GetInstance()->EndOfflineRendering();

    offlineRendering_ = false;
    offlineSongLastRow_ = 0;

    if (viewData_) {
        viewData_->songX_ = exportSaveSongX_;
        viewData_->songY_ = exportSaveSongY_;
        viewData_->songOffset_ = exportSaveSongOffset_;
        viewData_->chainRow_ = exportSaveChainRow_;
        viewData_->playMode_ = exportSavePlayMode_;
    }
    sequencerMode_ = exportSaveSequencerMode_;

    exportActive_ = false;
    exportOk_ = ok;
    Trace::Log("WAVExport", "offline export end ok=%d mode=%d firstRow=%d lastRow=%d slices=%d", ok ? 1 : 0, exportRenderMode_, exportFirstRow_, exportLastRow_, exportSlice_);
}

void Player::CancelSongExport() {
    if (!exportActive_) return;
    finishSongExport(false);
}

bool Player::IsExportingSong() const { return exportActive_; }

int Player::GetExportProgressPercent() const {
    if (!exportActive_) return exportOk_ ? 100 : 0;
    if (exportMaxSlices_ <= 0) return 0;
    int pct = (exportSlice_ * 100) / exportMaxSlices_;
    if (pct < 0) pct = 0;
    if (pct > 99) pct = 99;
    return pct;
}

int Player::GetExportRenderMode() const { return exportRenderMode_; }

bool Player::LastExportSucceeded() const { return exportOk_; }

bool Player::ExportSongToWav(int renderMode) {
    if (!BeginSongExportToWav(renderMode)) return false;
    while (IsExportingSong()) {
        ContinueSongExport(32);
    }
...
int Player::GetMasterPeakLeft() {
     return mixer_->GetMasterPeakLeft() ;
}

int Player::GetMasterPeakRight() {
     return mixer_->GetMasterPeakRight() ;
}

int Player::GetChannelPeak(int channel) {
     return mixer_->GetChannelPeak(channel) ;
}

bool Player::IsChannelClipped(int channel) {
     return mixer_->IsChannelClipped(channel) ;
}
        ...
        if (mixer_->IsChannelPlaying(i)) {
			...
			if (phrase!=0xFF) {
				if (gs->TriggerChannel(i)) { // If groove says it is time to play
					...
					unsigned char phraseVol=viewData_->song_->phrase_->volume_[phrase*16+pos] ;
					if (phraseVol!=0xFF) {
						I_Instrument *volInstrument=mixer_->GetInstrument(i) ;
						if (volInstrument) {
							volInstrument->ProcessCommand(i,I_CMD_VOLM,(ushort)phraseVol) ;
						}
					}
...
void Player::updateChainPos(int pos,int channel,int hop) {
	...
	if (viewData_->currentPlayPhrase_[channel] != 0xFF) {
...
void Player::updatePhrasePos(int pos,int channel) {
	...
	/* TREEFROG_START_GUARD: empty phrase/no row protection. */
    if ((phrase==0xFF)||(pos<0)||(pos>=16)) {
        timeToStart_[channel]=0 ;
        return ;
    }
...
// once all position have been updated
    ...
    if (offlineRendering_ && mode_ == PM_SONG) {
        int nextLinearPos = (viewData_->songPlayPos_[channel]) + 1;
        if (nextLinearPos > offlineSongLastRow_) {
            mixer_->StopChannel(channel);
            return;
        }
        unsigned char *nextChain = viewData_->song_->data_ + channel + 8 * nextLinearPos;
        if (*nextChain == 0xFF || viewData_->song_->chain_->data_[16 * (*nextChain)] == 0xFF) {
            mixer_->StopChannel(channel);
            return;
        }
    }
...
void Player::TreeFrogSoftStopStateOnly() {
    TreeFrogStartTrace("TREEFROG_SOFT_STOP_STATE_ONLY: enter");

    if (viewData_) {
        for (int i = 0; i < SONG_CHANNEL_COUNT; ++i) {
            QueueChannel(i, QM_NONE, 0, 0);
    ...
    TreeFrogStartTrace("TREEFROG_SOFT_STOP_STATE_ONLY: leave");
}
...
double Player::GetPlayTime() {
    ...
    double currentTime = mixer_->GetAudioOut()->GetStreamTime();
    double t = currentTime - startTime_;

    if (t < 0.0) {
        TreeFrogStartTrace("TREEFROG_GETPLAYTIME_CLAMP_NEGATIVE");
        return 0.0;
    }
...
#if defined(PLATFORM_TREEFROG)
    /* TREEFROG_V1_2_LISTEN_STREAM_ARM: ImportSampleDialog::preview() uses
       Player::StartStreaming(), not Player::Start(). V14/V1 audio render guard
       only arms audio from Player::Start(), so sample pre-listen was silent.
       Arm audio here for file streaming preview. */
    if (!offlineRendering_) TreeFrogAudioSetPlaybackArmed(1);
#endif
...
void Player::StartStreamingAt(const Path &path,int startFrame) {
#if defined(PLATFORM_TREEFROG)
    if (!offlineRendering_) TreeFrogAudioSetPlaybackArmed(1);
#endif
