...
// reserved (not used by these commands).  Item 5: the master-FX commands
// convert byte -> percent -> natural with the SAME descriptor curve as the
// Mixer UI (fxParamFromByte: LOG2 for time/frequency, LINEAR for gains), so
// a phrase byte and a UI percent produce identical values.
