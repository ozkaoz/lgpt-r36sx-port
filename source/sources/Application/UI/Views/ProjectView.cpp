...
#include "Application/UI/Views/ModalDialogs/NewProjectDialog.h"
...
// TREEFROG_PROJECT_RENAME_V2 (H38.6): the "Rename Project" action was
// removed from this menu. Renaming a project crashed when the port was
// re-entered from TreeFrogUI, so the rename now lives in the startup menu
// (SelectProjectDialog, R1+A) where no project is loaded and the rename can
// only touch the on-disk directory.
...
ProjectView::ProjectView(GUIWindow &w,ViewData *data):FieldView(w,data) {
    ...
    position._y += 2;
    a1 = new UIActionField("Audio Driver", ACTION_AUDIO_DRIVER, position);
    a1->AddObserver(*this);
    T_SimpleList<UIField>::Insert(a1);
...
# ifdef _64BIT
    ...
    switch (fourcc) {
        ...
        case ACTION_AUDIO_DRIVER: {
            DoModal(new AudioDriverModal(*this), AudioDriverModalApplyCallback);
            break;
        }
        case ACTION_QUIT: {
