...
int FileSystemService::Copy(const Path &src,const Path &dst)
{
    ...
    if (!isrc) {
        Trace::Error("FileSystemService::Copy source open failed: %s",
                     src.GetPath().c_str());
        return -1;
    }
    ...
    if (!idst) {
        Trace::Error("FileSystemService::Copy destination open failed: %s",
                     dst.GetPath().c_str());
        isrc->Close();
        delete isrc;
        return -1;
    ...
    bool ok=true;
    for (;;) {
        int count=isrc->Read(buffer,1,bufsize);
        if (count<0) {
            ok=false;
            break;
        }
        ...
        int written=idst->Write(buffer,1,count);
        if (written!=count) {
            Trace::Error("FileSystemService::Copy short write: expected %d got %d",
                         count,written);
            ok=false;
            break;
        }
