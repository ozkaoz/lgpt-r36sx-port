...
# Publish LGPT R36SX H38.4 (hardware feedback fixes: centered menus, phrase
# volume semantics, safe FX ranges, segmented mixer VU) to ozkaoz/lgpt-r36sx-port.
...
LOG="$LOG_ROOT/PUBLISH_H384_${TIMESTAMP}.log"
TMP_ROOT=""

mkdir -p "$LOG_ROOT" "$BACKUP_ROOT"
exec > >(tee -a "$LOG") 2>&1

CORE_SHA256="d01a8893b9d47c14136996fbc03574ad195a9f717243f0352f4e3b8ad69af5e1"
...
verify_resume_worktree() {
  ...
  (( unexpected == 0 )) ||
    fail "repository contains changes unrelated to H38.4; they were not modified"
...
validate_tree() {
  ...
  [[ -s "$sd_root/lgpt/otg/H38_4_ABI7_THREE_MODE.txt" ]] || fail "H38.4 marker missing in SD payload"
  grep -F 'VERSION=H38.4_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' "$sd_root/lgpt/otg/H38_4_ABI7_THREE_MODE.txt" >/dev/null ||
    fail "H38.4 marker content invalid"

  grep -F 'H38.4_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' \
  ...
  grep -F 'BACKGROUND value="0A0A18"' "$sd_root/lgpt/config.xml" >/dev/null ||
    fail "H38.4 palette missing in config.xml"
...
commit_release() {
  ...
  git -C "$REPO_ROOT" commit -m "Release H38.4: centered command menus, proportional phrase volume (--/00 silent), safe FX ranges, segmented mixer VU"
  git -C "$REPO_ROOT" tag -a "$TAG" -m "LGPT R36SX H38.4 fixes from hardware feedback"
...
build_asset() {
  ...
  asset="$REPO_ROOT/dist/LGPT_R36SX_H384_COPYROOT_3AUDIO_FULL_SOURCE.zip"
...
publish() {
  ...
  gh release create "$TAG" \
    ...
    --repo "$GITHUB_REPO" \
    --title "LGPT R36SX H38.4 — Menús centrados, volumen proporcional, FX seguros, VU segmentado" \
    --notes-file "$REPO_ROOT/docs/RELEASE_H38.4_ES.md" \
    --verify-tag \
    --latest
...
main() {
  local asset
  TMP_ROOT="$(mktemp -d "$HOME/lgpt-r36sx-publish-h384.XXXXXX")"
  ...
  echo "PUBLISH_RESULT=GITHUB_RELEASE_CREATED_H384"
}
