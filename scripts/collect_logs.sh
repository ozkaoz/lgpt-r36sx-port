...
mkdir -p "$WORK/runtime_state" "$WORK/records"
{
  ...
  for f in "$SD/cubegm/cores/lgpt_r36sx_port_libretro.so" "$SD/lgpt/otg/bin/r36s_u241_usb_audio_io" "$SD/lgpt/otg/modules/4.4.186-release/u2_38au8_sync_uac2/usb_f_uac2.ko"; do [[ -f "$f" ]] && { file "$f"; sha256sum "$f"; }; done
} > "$WORK/STATE.txt"
