...
# Publish LGPT R36SX H38.3 (interface and controls tested on hardware) to ozkaoz/lgpt-r36sx-port.
...
LOG="$LOG_ROOT/PUBLISH_H383_${TIMESTAMP}.log"
TMP_ROOT=""

mkdir -p "$LOG_ROOT" "$BACKUP_ROOT"
exec > >(tee -a "$LOG") 2>&1

CORE_SHA256="ca3baa9efb778f5168f53df3e3b089f0d85dde5bd0056d9539cded22bba80e75"
...
verify_resume_worktree() {
  ...
  (( unexpected == 0 )) ||
    fail "repository contains changes unrelated to H38.3; they were not modified"
...
validate_tree() {
  ...
  [[ -s "$sd_root/lgpt/otg/H38_3_ABI7_THREE_MODE.txt" ]] || fail "H38.3 marker missing in SD payload"
  grep -F 'VERSION=H38.3_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' "$sd_root/lgpt/otg/H38_3_ABI7_THREE_MODE.txt" >/dev/null ||
    fail "H38.3 marker content invalid"

  grep -F 'H38.3_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' \
  ...
  grep -F 'BACKGROUND value="0A0A18"' "$sd_root/lgpt/config.xml" >/dev/null ||
    fail "H38.3 palette missing in config.xml"
...
commit_release() {
  ...
  git -C "$REPO_ROOT" commit -m "Release H38.3: sidebar bottom-left, live mixer VU, editable phrase volume, 3-char notes, palette, R2+A FX / R1+A solo"
  git -C "$REPO_ROOT" tag -a "$TAG" -m "LGPT R36SX H38.3 interface and controls tested on hardware"
...
build_asset() {
  ...
  asset="$REPO_ROOT/dist/LGPT_R36SX_H383_COPYROOT_3AUDIO_FULL_SOURCE.zip"
...
publish() {
  ...
  gh release create "$TAG" \
    ...
    --repo "$GITHUB_REPO" \
    --title "LGPT R36SX H38.3 — Sidebar, VU en vivo, volumen editable, colores, R1/R2" \
    --notes-file "$REPO_ROOT/docs/RELEASE_H38.3_ES.md" \
    --verify-tag \
    --latest
...
main() {
  local asset
  TMP_ROOT="$(mktemp -d "$HOME/lgpt-r36sx-publish-h383.XXXXXX")"
  ...
  echo "PUBLISH_RESULT=GITHUB_RELEASE_CREATED_H383"
}
