...
# Publish LGPT R36SX H38.2 (three-mode audio: Local/Windows/Android) to ozkaoz/lgpt-r36sx-port.
...
LOG="$LOG_ROOT/PUBLISH_H382_${TIMESTAMP}.log"
TMP_ROOT=""

mkdir -p "$LOG_ROOT" "$BACKUP_ROOT"
exec > >(tee -a "$LOG") 2>&1

CORE_SHA256="9f37e01725a084e291d1d50bb9dfb9493f5fc4473caa86d3c7f1cbe3cd408d8e"
...
verify_resume_worktree() {
  ...
  (( unexpected == 0 )) ||
    fail "repository contains changes unrelated to H38.2; they were not modified"
...
validate_tree() {
  ...
  [[ -s "$sd_root/lgpt/otg/H38_2_ABI7_THREE_MODE.txt" ]] || fail "H38.2 marker missing in SD payload"
  grep -F 'VERSION=H38.2_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' "$sd_root/lgpt/otg/H38_2_ABI7_THREE_MODE.txt" >/dev/null ||
    fail "H38.2 marker content invalid"

  grep -F 'H38.2_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' \
...
commit_release() {
  ...
  git -C "$REPO_ROOT" commit -m "Release H38.2: three-mode audio Local/Windows/Android, mixer FX, sidebar, phrase FX"
  git -C "$REPO_ROOT" tag -a "$TAG" -m "LGPT R36SX H38.2 three-mode audio (Local/Windows/Android) + mixer FX + sidebar"
...
build_asset() {
  ...
  asset="$REPO_ROOT/dist/LGPT_R36SX_H382_COPYROOT_3AUDIO_FULL_SOURCE.zip"
...
publish() {
  ...
  gh release create "$TAG" \
    ...
    --repo "$GITHUB_REPO" \
    --title "LGPT R36SX H38.2 — Audio Local/Windows/Android + Mixer FX + Sidebar" \
    --notes-file "$REPO_ROOT/docs/RELEASE_H38.2_ES.md" \
    --verify-tag \
    --latest
...
main() {
  local asset
  TMP_ROOT="$(mktemp -d "$HOME/lgpt-r36sx-publish-h382.XXXXXX")"
  ...
  echo "PUBLISH_RESULT=GITHUB_RELEASE_CREATED_H382"
}
