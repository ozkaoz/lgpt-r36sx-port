...
# Publish LGPT R36SX H38.5 (FX3 removal, centered grids, 1 = 100% scale, dense
# mixer VU, rename project) to ozkaoz/lgpt-r36sx-port.
...
LOG="$LOG_ROOT/PUBLISH_H385_${TIMESTAMP}.log"
TMP_ROOT=""

mkdir -p "$LOG_ROOT" "$BACKUP_ROOT"
exec > >(tee -a "$LOG") 2>&1

CORE_SHA256="ed3c8afb2cf215773e3609e73b843fa6b4cdd02827b8b62acef66ba4d5f1d2f2"
...
verify_resume_worktree() {
  ...
  (( unexpected == 0 )) ||
    fail "repository contains changes unrelated to H38.5; they were not modified"
...
validate_tree() {
  ...
  [[ -s "$sd_root/lgpt/otg/H38_5_ABI7_THREE_MODE.txt" ]] || fail "H38.5 marker missing in SD payload"
  grep -F 'VERSION=H38.5_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' "$sd_root/lgpt/otg/H38_5_ABI7_THREE_MODE.txt" >/dev/null ||
    fail "H38.5 marker content invalid"

  grep -F 'H38.5_ABI7_THREE_MODE_LOCAL_WINDOWS_ANDROID_SAFE_FRONTEND' \
  ...
  grep -F 'BACKGROUND value="0A0A18"' "$sd_root/lgpt/config.xml" >/dev/null ||
    fail "H38.5 palette missing in config.xml"
...
commit_release() {
  ...
  git -C "$REPO_ROOT" commit -m "Release H38.5: remove FX3, centered grids with headers, 1 = 100% volume/FX, dense mixer VU, rename project"
  git -C "$REPO_ROOT" tag -a "$TAG" -m "LGPT R36SX H38.5 fixes from hardware feedback"
...
build_asset() {
  ...
  asset="$REPO_ROOT/dist/LGPT_R36SX_H385_COPYROOT_3AUDIO_FULL_SOURCE.zip"
...
publish() {
  ...
  gh release create "$TAG" \
    ...
    --repo "$GITHUB_REPO" \
    --title "LGPT R36SX H38.5 — FX3 eliminado, cuadrículas centradas, 1 = 100%, VU denso, Rename Project" \
    --notes-file "$REPO_ROOT/docs/RELEASE_H38.5_ES.md" \
    --verify-tag \
    --latest
...
main() {
  ...
  echo "PUBLISH_RESULT=GITHUB_RELEASE_CREATED_H385"
}
