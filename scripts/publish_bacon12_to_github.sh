...
# Publish LGPT R36SX Bacon 1.2 (Mixer Dev) to ozkaoz/lgpt-r36sx-port.
...
LOG="$LOG_ROOT/PUBLISH_BACON12_${TIMESTAMP}.log"
TMP_ROOT=""
...
verify_resume_worktree() {
  ...
  (( unexpected == 0 )) ||
    fail "repository contains changes unrelated to Bacon 1.2; they were not modified"
...
commit_release() {
  ...
  git -C "$REPO_ROOT" commit -m "Release Bacon 1.2: Mixer Dev (V6-V16). Medidores estéreo L/R pre-clip, undo/redo global L1+X/R1+X en Song/Chain/Phrase/Table, A+B reset option, Solo/Mute deshacibles, barras VU bottom-up con banda roja, menú L1+A del mixer, R2+Y normalizar sample en el chopper, Help por secciones y perfil USB STEREO_48K."
  git -C "$REPO_ROOT" tag -a "$TAG" -m "LGPT R36SX Bacon 1.2 release"
...
build_asset() {
  ...
  asset="$REPO_ROOT/dist/LGPT_R36SX_BACON12_COPYROOT_MIXERDEV.zip"
...
publish() {
  ...
  gh release create "$TAG" \
    ...
    --repo "$GITHUB_REPO" \
    --title "LGPT R36SX - Bacon 1.2 - Mixer Dev" \
    --notes-file "$REPO_ROOT/docs/RELEASE_BACON_1.2_ES.md" \
    --verify-tag \
    --prerelease
...
main() {
  local asset
  TMP_ROOT="$(mktemp -d "$HOME/lgpt-r36sx-publish-bacon12.XXXXXX")"

  echo "============================================================"
  echo "PUBLISH LGPT R36SX $VERSION — MIXER DEV"
  echo "============================================================"
  ...
  echo "PUBLISH_RESULT=GITHUB_RELEASE_CREATED_BACON12"
}
