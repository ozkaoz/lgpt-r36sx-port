...
# Publish LGPT R36SX Bacon 1.1 (Audio Driver Refact, Sampler Audio Added) to
...
LOG="$LOG_ROOT/PUBLISH_BACON11_${TIMESTAMP}.log"
TMP_ROOT=""

mkdir -p "$LOG_ROOT" "$BACKUP_ROOT"
exec > >(tee -a "$LOG") 2>&1

CORE_SHA256="5685150957fbdfcaca9d38afcf5d4753114c48e2b0b40abe34cfee2130f7d1cb"
DAEMON_SHA256="53258f2b8b3749c866af248814eb147f0762a1b17cfffb644adb573167b52815"
SP404_SHA256="924c48436086c2b84897a2bde0547e5e8b9381a60a4845ebcd82fc8aab47961a"
...
verify_resume_worktree() {
  ...
  (( unexpected == 0 )) ||
    fail "repository contains changes unrelated to Bacon 1.1; they were not modified"
...
validate_tree() {
  ...
  grep -F 'VERSION=H38.7_ABI7_BACON11_AUDIO_DRIVER_REFACT_SAMPLER' "$sd_root/lgpt/otg/H38_7_ABI7_THREE_MODE.txt" >/dev/null ||
    fail "H38.7 marker content invalid"
  grep -F 'SP404_DAEMON_SHA256=924c48436086c2b84897a2bde0547e5e8b9381a60a4845ebcd82fc8aab47961a' "$sd_root/lgpt/otg/H38_7_ABI7_THREE_MODE.txt" >/dev/null ||
    fail "H38.7 marker SP404 sha missing"

  grep -F 'BACON_1.1_AUDIO_DRIVER_REFACT_SAMPLER_AUDIO' \
...
commit_release() {
  ...
  git -C "$REPO_ROOT" commit -m "Release Bacon 1.1: audio driver refact (passthrough puro 48k, sin resampler) + Sampler audio estable, FIFO_DUMP y fix overflow (U2.53.x). Four-mode verified: Windows/Local/Android/Sampler."
  git -C "$REPO_ROOT" tag -a "$TAG" -m "LGPT R36SX Bacon 1.1 release"
...
build_asset() {
  ...
  asset="$REPO_ROOT/dist/LGPT_R36SX_BACON11_COPYROOT_AUDIO_DRIVER_REFACT_SAMPLER.zip"
...
publish() {
  ...
  gh release create "$TAG" \
    ...
    --repo "$GITHUB_REPO" \
    --title "LGPT R36SX - Bacon 1.1 - Audio Driver Refact, Sampler Audio Added" \
    --notes-file "$REPO_ROOT/docs/RELEASE_BACON_1.1_ES.md" \
    --verify-tag \
    --latest
...
main() {
  local asset
  TMP_ROOT="$(mktemp -d "$HOME/lgpt-r36sx-publish-bacon11.XXXXXX")"

  echo "============================================================"
  echo "PUBLISH LGPT R36SX $VERSION — AUDIO DRIVER REFACT + SAMPLER"
  echo "============================================================"
  ...
  echo "PUBLISH_RESULT=GITHUB_RELEASE_CREATED_BACON11"
}
